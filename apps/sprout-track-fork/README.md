...
A hardened, rootless, read-only-root-filesystem re-layer of the
[`TomGrozev/sprout-track`](https://github.com/TomGrozev/sprout-track) **fork** of
...
Unlike the sibling `sprout-track` app (which re-layers the published
`sprouttrack/sprout-track` image), this image is built **from source** at a
Renovate-pinned commit of the fork's `main` branch
(`https://github.com/TomGrozev/sprout-track`): Renovate tracks `main`'s HEAD as a
git-refs digest pin and auto-merges the bump, which triggers a rebuild.
The fork has no published image for `main` (it publishes to Docker Hub only on
release), so the fork's own `Dockerfile` is built from the pinned commit and then
hardened here.
...
controllers:
  app:
    ...
    containers:
      app:
        image:
          repository: ghcr.io/tomgrozev/bedrock-containers/sprout-track-fork
