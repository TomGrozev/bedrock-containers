# Security — Known Vulnerabilities in `sprout-track-fork`
...
| `CVE-2026-82209` | False positive on Wolfi curl 8.22.0-r2: affected range is <= 8.21.0 and the fix shipped IN 8.22.0 (curl rates this Low); some Grype DB vintages lack fixed-in data for this ID and flag the patched package. curl in this image only pings the local app. See apps/sprout-track-fork/SECURITY.md |
